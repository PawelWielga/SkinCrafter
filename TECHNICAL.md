...
Pushes to `main` and `dev` are deployable only when both workflows have completed successfully for the exact same commit SHA. Standalone deployment paths are defined once in `apps/standalone/deployment.config.json`. Normal production and local builds use the `production` entry; validated `dev` pushes set `SKINCRAFTER_DEPLOYMENT=dev`, so Vite, the DEV badge and the generated Pages fallback all derive behavior from that same deployment selection. The dev preview must use a dedicated one-segment top-level base path so it cannot overlap generated production-root output; the standalone build and deployment workflow both enforce that constraint. The `Build` workflow uploads the already validated `apps/standalone/dist` directory as a short-lived Actions artifact for deployable branch pushes.

`.github/workflows/gh-pages-dev.yml` publishes the validated preview into the configured dev destination and updates the root `404.html` in the same serialized `gh-pages` change, so a base-path rename cannot move the preview without moving its deep-link fallback. It also writes generated deployment state to `gh-pages`; `.github/workflows/gh-pages.yml` preserves that actively deployed preview and regenerates the production fallback against the same active preview path, even while `main` and `dev` temporarily contain different configuration revisions. Both deploy workflows share one concurrency group so they cannot update `gh-pages` simultaneously. The current configured preview is exposed at [https://skincrafter.dihor.pl/dev/](https://skincrafter.dihor.pl/dev/). Write permission is granted only to deployment jobs. The third-party Pages publishing action that runs with that write permission is pinned to a reviewed full upstream commit SHA, with its corresponding release version kept as a workflow comment for maintainability.
...
GitHub Pages uses `apps/standalone/scripts/create-pages-fallback.mjs` to generate the root SPA fallback from the deployment configuration. A direct request or browser refresh first redirects through the active production or preview entry point, then restores the original URL before React Router starts. Dev deployment updates the root fallback together with the preview, and main deployment preserves that active preview routing state. This keeps creator, `/mcskinview` and localized Not Found routing functional on both deployments without requiring a full document reload during normal client-side navigation.
