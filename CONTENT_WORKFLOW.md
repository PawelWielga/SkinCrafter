...
| Sex | No independent texture folder | Derived from model variants in `src/data/raceTextureMap.ts` | Male, Female | Sex availability follows the selected production race/model variant. Internal `Template` uses `None`, but that value is not selectable production content. Do not create a disconnected sex texture registry. |
